...
public class DbAccess
{
    ...
    ///查询是否包含指定数据表
    ///</summary>
    public SqliteDataReader ExistTables(string tableName)
    {
		string query="SELECT * FROM sqlite_master" + " WHERE name="+"'"+tableName+"'";
		return ExecuteQuery(query);

	}

	public SqliteDataReader DataCount(string tableName)
    {
		string query = "SELECT * FROM " + tableName;
		return ExecuteQuery(query);
	}
